...
class WebsiteAccount(website_account):
    ...
    def account(self, **kw):
        ...
        response.qcontext.update({"contract_count": len(contracts)})
        return response
...
class WebsiteContract(http.Controller):
    ...
    def portal_contract(self, contract):
        action = request.env.ref("contract.action_account_analytic_overdue_all")
        values = {"user": request.env.user, "contract": contract, "action": action.id}
        return request.render("website_portal_contract.website_contract", values)
